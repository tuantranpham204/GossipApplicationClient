      ...
      {user && (
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-4">
            {[
                ...
                { path: '/requests', icon: UserPlus, title: 'Requests', badge: true },
                { path: '/notifications', icon: Bell, title: 'Notifications' },
            ].map((item) => (
                <Link key={item.path} to={item.path} className="relative group">
                    <motion.div
                        whileHover={{ scale: 1.1, paddingBottom: "8px", paddingTop: "8px" }}
                        whileTap={{ scale: 0.95 }}
                ...
                <AnimatePresence>
                  {isProfileOpen && (
                    ...
                    >
                      ...
                      >
                        <LogOut size={18} strokeWidth={2} className="text-red-400 group-hover:text-red-500" />
