...
import { Settings, Grid, Bookmark, UserSquare, MoreHorizontal, Heart, MessageCircle, Mail, Pencil, Archive, User, Lock, Camera } from 'lucide-react';
import Header from '../components/layout/Header';
import Footer from '../components/layout/Footer';
import { useHostProfileQuery, useGuestProfileQuery } from '../services/user.service';
...
const ProfilePage = () => {
    ...
    // We call both hooks but enable only one.
    const hostQuery = useHostProfileQuery(userId, { enabled: isHost });
    const guestQuery = useGuestProfileQuery(userId, { enabled: !isHost });

    const { data: userProfile, isLoading, error } = isHost ? hostQuery : guestQuery;
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4 text-slate-500">
                ...
                <Link to="/" className="text-blue-500 font-semibold hover:text-blue-700">Go Home</Link>
            </div>
        );
    }
                        ...
                        <div className="relative w-24 h-24 md:w-40 md:h-40 rounded-full p-[2px] bg-linear-to-tr from-yellow-400 via-red-500 to-purple-600">
                             <div className="w-full h-full rounded-full border-2 border-white bg-slate-50 overflow-hidden">
                                {user.avatar_data?.url ? (
                                    <img src={user.avatar_data.url} alt={user.username} className="w-full h-full object-cover" />
                                ) : (
                    ...
                    <div className="flex-1 flex flex-col gap-5 w-full">
                        ...
                        <div className="flex flex-col justify-between md:flex-row items-center md:items-start gap-4">
                            <div className="flex flex-col gap-1">
                                <h1 className="text-xl md:text-2xl font-light text-slate-800">{user.username}</h1>
                                <span className="text-sm font-medium text-slate-600 block md:hidden">{user.first_name} {user.last_name}</span>
                            </div>
                            ...
                            {isHost && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <button className="p-2 text-slate-800 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" title="Archive">
                                        <Archive size={20} />
                                    </button>
                                    <button className="p-2 text-slate-800 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" title="Settings">
                                        <Settings size={20} />
                                    </button>
                                </div>
                            )}
                        ...
                        <ul className="flex justify-center md:justify-start items-center gap-8 text-base">
                            <li className="flex gap-1">
                                <span className="font-semibold text-slate-900">{user.posts_amt || -1}</span>
                                <span className="text-slate-600">posts</span>
                            </li>
                            <li className="flex gap-1">
                                <span className="font-semibold text-slate-900">{user.friends_amt || 0}</span>
                                <span className="text-slate-600">friends</span>
                            </li>
                            <li className="flex gap-1">
                                <span className="font-semibold text-slate-900">{user.followers_amt || 0}</span>
                                <span className="text-slate-600">followers</span>
                            </li>
                            <li className="flex gap-1">
                                <span className="font-semibold text-slate-900">{user.following_amt || 0}</span>
                        ...
                        <div className="text-center md:text-left mt-4 w-full">
                            <h2 className="hidden md:block font-semibold text-slate-900 text-base mb-2">{user.first_name} {user.last_name}</h2>
                            ...
                            <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 text-sm text-slate-600">
                                {(isHost || user.is_email_public) && user.email && (
                                    <div className="flex items-center gap-2" title={user.is_email_public ? "Email" : "Private Email"}>
                                        {user.is_email_public ? (
                                            <Mail size={16} className="text-slate-400" />
                                        ) : (
                                            <Lock size={16} className="text-slate-400" />
                                        )}
                                        <span className={`font-medium ${user.is_email_public ? "text-slate-700" : "text-slate-400"}`}>
                                            {user.email}
                                        </span>
                                    </div>
                                )}
                                
                                {(isHost || user.is_gender_public) && (user.gender !== null && user.gender !== undefined) && (
                                    <div className="flex items-center gap-2 capitalize" title={user.is_gender_public ? "Gender" : "Private Gender"}>
                                        {user.is_gender_public ? (
                                            <User size={16} className="text-slate-400" />
                                        ) : (
                                            <Lock size={16} className="text-slate-400" />
                                        )}
                                        <span className={`font-medium ${user.is_gender_public ? "text-slate-700" : "text-slate-400"}`}>
                                            {user.gender}
                                        </span>
                                    </div>
                                )}

                                {(isHost || user.is_rel_status_public) && (user.relationship_status !== null && user.relationship_status !== undefined) && (
                                    <div className="flex items-center gap-2 capitalize" title={user.is_rel_status_public ? "Relationship Status" : "Private Relationship Status"}>
                                        {user.is_rel_status_public ? (
                                            <Heart size={16} className="text-slate-400" />
                                        ) : (
                                            <Lock size={16} className="text-slate-400" />
                                        )}
                                        <span className={`font-medium ${user.is_rel_status_public ? "text-slate-700" : "text-slate-400"}`}>
                ...
                {activeTab === 'posts' ? (
                    <div className="grid grid-cols-3 gap-1 sm:gap-4 mt-2">
                        {/* Placeholder Items */}
                         {[1, 2, 3, 4, 5, 6].map((item) => (
                            <div key={item} className="aspect-square relative group bg-slate-100 hover:bg-slate-200 transition-colors cursor-pointer overflow-hidden">
                    ...
                    <div className="py-20 flex flex-col items-center justify-center text-slate-500">
                        <div className="w-16 h-16 rounded-full border-2 border-slate-300 flex items-center justify-center mb-4">
                            ...
                            {activeTab === 'tagged' && <UserSquare size={32} strokeWidth={1} />}
                        </div>
